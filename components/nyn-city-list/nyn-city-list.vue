<template>
	<view>
		<!-- 搜索框 -->
		<view class='search'>
			<input type='text' placeholder='请输入城市名称' style="font-size:24rpx; padding: 10rpx 24rpx;" @input='bindinputCitys' />
		</view>
		<!-- 热门城市 -->
		<view class="hot" v-if="hot">
			<view v-for="item in hotList" :key="item.id" @tap="bindCity(item)" :data-city="item.name">{{item.name}}
			</view>
		</view>
		<!-- 首字母检索 -->
		<view class="searchLetter touchClass">
			...
			<view v-for="item in searchLetter" :key="item.name" :data-letter="item.name" :style="{height:`${itemH}px`}"
				@touchstart.stop="searchStart" @touchmove.stop="searchMove" @touchend.stop="searchEnd">{{item.name}}
			</view>
		</view>
		...
		<scroll-view scroll-y="true" :style="{height:`${winHeight}px`}" :scroll-top="scrollTop" v-if="!search">
			<!-- 循环城市数据 -->
			<view v-for="(value,key) in cityList" :key="key">
				<!-- 循环首字母检索信息 -->
				<view class="list_letter">{{key}}</view>
				<!-- 城市ID城市名称 -->
				<view class="item_city" v-for="item in value" :key="item.id" :data-city="item.name"
					@tap="bindCity(item)">{{item.name}}</view>
			</view>
		</scroll-view>
		<!-- 搜索列表 -->
		<view class='list' v-if="search">
			<view class='text' v-for="item in cyits" :key="item.id" @tap="bindCity(item)" :data-city="item.name">
				{{item.name}}</view>
		</view>
	</view>
</template>
			...
			bindinputCitys(e) {
				...
				for (let key in cities) {
					for (let i = 0; i < cities[key].length; i++) {
						if (cities[key][i].name.search(e.detail.value) != -1 || cities[key][i].spell.search(e.detail
							.value) != -1) {
